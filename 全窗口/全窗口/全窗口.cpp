...
HWND InstProce, InstText, InstTip;
...
typedef TCHAR FileList[MAX_PATH];
typedef TCHAR FileName[FILENAME_MAX];
...
BTN_SELECTPATH=561,BTN_NEXTPAGE=562,BTN_BACKPAGE=653;
...
void ShowPathSelect(bool sta = FALSE) {
    if (sta) {
        ...
        PathEdit=CreateWindow(_T("EDIT"), L"D:\\TaruCraft", WS_TABSTOP | WS_CHILD | WS_BORDER | SS_LEFT | SS_CENTERIMAGE | ES_READONLY,
...
BOOL MakeDir(WCHAR *Path) {//创建多级目录
    UINT32 count = 0;
    WCHAR OrginPath[MAX_PATH + 1] = L"";
    struct _stat s;
    wcscpy_s(OrginPath, Path);//复制路径到更长的字符串,用于之后的算法
    WCHAR TempPath[MAX_PATH+2] = L"";
    for (count = 0;count < wcslen(Path)+1;++count) {//遍历路径字符串逐级提取目录尝试创建
        if ((OrginPath[count] != L'\\')&&(count != wcslen(Path))) {
            TempPath[count] = OrginPath[count];
        }
        else {
            _wmkdir(TempPath);//创建单个目录
            TempPath[count] = OrginPath[count];
        }
    }
...
BOOL RegInstallInfo(void) {//写入注册表安装信息
    ...
    ::RegSetValueEx(RegKey, L"DisplayName", 0, REG_SZ, (byte*)DisplayName,sizeof(DisplayName)*sizeof(WCHAR));
    WCHAR Publisher[] = L"塔里木大学方块创作社";
    RegSetValueEx(RegKey, L"Publisher", 0, REG_SZ, (byte*)Publisher, sizeof(Publisher) * sizeof(WCHAR));
    WCHAR Version[] = L"1.0";
    RegSetValueEx(RegKey, L"Version", 0, REG_SZ, (byte*)Version, sizeof(Version) * sizeof(WCHAR));
    RegSetValueEx(RegKey, L"InstallPath", 0, REG_SZ, (byte*)InstallPath, sizeof(InstallPath) * sizeof(WCHAR));
    WCHAR UninstallString[MAX_PATH] = L"";
    wcscpy_s(UninstallString, InstallPath);
    wcscat_s(UninstallString, L"\\Uninstall.exe");
    RegSetValueEx(RegKey, L"UninstallString", 0, REG_SZ, (byte*)UninstallString, sizeof(UninstallString) * sizeof(WCHAR));
    wcscat_s(UninstallString, L",2");
    RegSetValueEx(RegKey, L"DisplayIcon", 0, REG_SZ, (byte*)UninstallString, sizeof(UninstallString) * sizeof(WCHAR));
...
void MkShellLink(LPCWSTR From, LPCWSTR Dir, LPCWSTR Name, LPCWSTR Argv, LPCWSTR Icon,CHAR IcoCount=0)
{
...
int InstthreadProc() {
    ...
    MkShellLink(Tmp, desk_path, Name, Argv, Tmp2,2);
    _wmkdir(StartMenu);
    wcscat_s(StartMenu, Dir);
    MkShellLink(Tmp, StartMenu, Name, Argv, Tmp2,2);
    wcscpy_s(Tmp, L"");
    wcscpy_s(Tmp, InstallPath);
    ...
    MkShellLink(Tmp, StartMenu, Name, Argv, Tmp2);
    Sleep(3 * 1000);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_UNZIP:
        if (Step < 2) {
            ...
            SendMessage(PathEdit, WM_GETTEXT, 32, (LPARAM)InstallPath);
            StepGoto(++Step);
            StartInst();
        }
        else if (Step == 4) {
